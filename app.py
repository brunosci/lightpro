              ...
              try:
                ...
                for index, row in stock_data.iterrows():
                    ...
                    if consecutive_true_count == 3 and not in_trade:
                        ...
                        drawdown = row['Low']
                    elif in_trade and consecutive_false_count == 3:
                        in_trade = False
                        exit_price = row['Close']
                        exit_price = round(exit_price, 2)
                        trades_sell.append(exit_price)
                ...
                trades = pd.DataFrame({'Buy':trades_buy, 'Sell': trades_sell, 'Dias': trades_periodo, 'Drawdown': trades_drawdown, 'Highest': trades_high})
                trades['Return'] = (trades['Sell'] / trades['Buy']-1)*100
                trades['Return']= round(trades.Return,2)
                ...
                capital = capital - 100
                capital = round(capital,2)
                st.markdown(f"<h6 style='text-align: left; color: white; background-color: #006400; padding: 10px; border-radius: 0px;'></h6>", unsafe_allow_html=True)         
                st.title(j)
